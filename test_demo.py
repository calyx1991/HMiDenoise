...
def select_model(args, device):
    ...
    if model_id == 22:
        # SGN test
        from models.team22_DHAT import HAT_v1
        name, data_range = f"{model_id:02}_DHAT", 1.0
        model_path = os.path.join('model_zoo', 'team22_DHAT.pth')
        model = HAT_v1()

        state_dict = torch.load(model_path, map_location=lambda storage, loc: storage)["params_ema"]
